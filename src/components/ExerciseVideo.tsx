...
export default function ExerciseVideo({
  ...
  const PlaceholderContent = () => (
    <div className={cn(
      "flex flex-col items-center justify-center bg-gradient-to-br from-slate-100 to-slate-300 dark:from-slate-700 dark:to-slate-900 text-slate-600 dark:text-slate-400",
      sizeClasses[size]
    )}>
      <PlayCircle className={cn(iconSizes[size], "mb-1 text-slate-500 dark:text-slate-400")} />
      {size !== 'small' && (
    ...
    >
      {!isVisible ? (
        ...
        <div className={cn(
          ...
          <Video className={cn(iconSizes[size], "text-slate-400")} />
        </div>
      ) : !src || hasError ? (
